...
public class Drag : MonoBehaviour
{
    ...
    public void DragHandler(BaseEventData data)
    {
        ...
        window.transform.position = canvas.transform.TransformPoint(position + offset); // Changes the position to follow the mouse depending on the Vector2 position + offset
    ...
    public void OnPointerDown(BaseEventData data)
    {
        ...
        offset = (Vector2)window.GetComponent<RectTransform>().position - pointerData.position; // Move the window around depending on where the mouse is (offset)
    }
}
